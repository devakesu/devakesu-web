...
export const EFFECTIVE_DATE = '2026-02-10';
...
When configured by the site owner, this site uses Google Analytics 4 via server-side Measurement Protocol to understand how visitors interact with the portfolio. **Google Signals is enabled** in those cases, which means:
...
**Important:** While I don't collect personal data, **if analytics is enabled, Google Analytics with Google Signals MAY associate your visit with your Google account** if you're signed in, which could include your name, email, and other Google profile information.
...
**Google Analytics 4 (with Google Signals - When Enabled):**
When the site owner has configured analytics:
...
**Opt-Out of Analytics (When Enabled):**
If analytics is configured on this site:
...
**Data Deletion:** Since this is a portfolio site with no user accounts, there's no personal data stored by me to delete. If analytics is enabled, Google Analytics retains data according to their retention policies. You can request deletion via [Google's Data Deletion Request](https://support.google.com/analytics/answer/9450800).

**Contact:** Questions or concerns? Email me at ${CONTACT_EMAIL}

### Data Retention

* If analytics is enabled: Analytics data is retained according to Google Analytics 4 defaults (2-14 months)
